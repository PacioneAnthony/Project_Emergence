"""Run the C1 margin probe (D-060, step 3).

    python -m scripts.research.c1_probe --subspace dev      # debugging, as often as needed
    python -m scripts.research.c1_probe --freeze            # before the bank: hash and archive the sources
    python -m scripts.research.c1_probe --subspace bank     # once, after the freeze

Definitions and thresholds: docs/research/c1_journal.md, entry 1. The bank refuses
to run unless every frozen source still matches its hash, and refuses to run a
second time: a bank played twice is a bank chosen.
"""
...
from learning.c1_probe import POLICIES, WITNESSES, play_episode, probe_seeds, verdict
from learning.c1_task import C1Config

ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "data/processed/experiments/c1_probe"
MANIFEST = ROOT / "docs/research/c1_probe_manifest.json"
PUBLISHED = ROOT / "docs/research/c1_probe_results.json"
COUNTS = {"dev": 10, "bank": 60}
THRESHOLD_COMMIT = "6cc7252"
FROZEN = (
...
def run(subspace: str, count: int) -> dict:
    ...
    for i, seed in enumerate(seeds, start=1):
        try:
            ...
            print(f"  [{i:2d}/{count}] piece {seed} ecartee par un garde : {exc}", flush=True)
            continue
        rows.append(row)
        marks = "   ".join(
        ...
        print(f"  [{i:2d}/{count}] piece {seed:>10}{' brassee' if row['moved'] else '        '}  {marks}", flush=True)
...
def main() -> int:
    ...
    if args.subspace == "bank" and count != COUNTS["bank"]:
        raise SystemExit("la banque compte 60 pieces, fixees dans le journal")
