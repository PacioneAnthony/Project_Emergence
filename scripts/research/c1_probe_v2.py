"""Run the C1 margin probe, version 2 (D-060, step 3).

    python -m scripts.research.c1_probe_v2 --subspace dev      # debugging, as often as needed
    python -m scripts.research.c1_probe_v2 --freeze            # before the bank: hash and archive the sources
    python -m scripts.research.c1_probe_v2 --subspace bank     # once, after the freeze

Definitions and thresholds: docs/research/c1_journal.md, entry 4, committed in
86a03c8 before any version 2 code existed. The bank refuses to run unless every
frozen source still matches its hash, and refuses to run a second time: a bank
played twice is a bank chosen.

The freeze covers version 1's nine sources as well as version 2's three. They are
not used any differently -- they are hashed to certify that version 2 ran against
a substrate whose bytes never moved.
"""
...
from learning.c1_probe_v2 import POLICIES, WITNESSES, play_episode, probe_seeds, verdict
from learning.c1_task import C1Config

ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "data/processed/experiments/c1_probe_v2"
MANIFEST = ROOT / "docs/research/c1_probe_v2_manifest.json"
PUBLISHED = ROOT / "docs/research/c1_probe_v2_results.json"
COUNTS = {"dev": 10, "bank": 200}
THRESHOLD_COMMIT = "86a03c8"
FROZEN = (
    # Version 2's own sources.
    "learning/c1_probe_v2.py",
    "learning/c1_task_v2.py",
    "scripts/research/c1_probe_v2.py",
    # Version 1's, reused unchanged and hashed to prove it.
...
def main() -> int:
    ...
    if args.subspace == "bank" and count != COUNTS["bank"]:
        raise SystemExit("la banque compte 200 pieces, fixees dans le journal")
