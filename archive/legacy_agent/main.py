...
def cortex_process(cortex_brain):
    ...
    while True:
        ...
        # 2. Réfléchir (Lent) - Version robuste pour Ollama
        try:
            full_response = cortex_brain.think(situation_text)
            # On vérifie si c'est un dictionnaire ou une string brute
            if isinstance(full_response, dict) and 'message' in full_response:
                strategy = full_response['message']['content']
            else:
                strategy = str(full_response)
        except Exception as e:
            strategy = "Erreur de pensée..."
    ...
    try:
        while True:
            ...
            if arduino_serial and arduino_serial.in_waiting > 0:
                try:
                    # L'Aspirateur : On lit toutes les lignes en attente une par une
                    # Cela vide le buffer instantanément et met à jour avec les valeurs les plus fraîches.
                        ...
                        if line and '|' in line:
                            ...
                            for part in parts:
                                if part.count(':') == 1:
                                    key, val = part.split(':')
                                    try:
                                        if key == 'P': raw_piezo = float(val)
                                        if key == 'D': raw_dist = float(val)
                                    except ValueError:
                                        continue
                except Exception as e:
                    pass
            ...
            # Le Cervelet décide d'une vitesse de rotation
            cmd_speed = action[0] * 0.1
            
            # On met à jour la position interne (entre 0.0 et 1.0)
            body_state["last_servo_pos"] = np.clip(body_state["last_servo_pos"] + cmd_speed, 0.0, 1.0)

            # --- NOUVEAU : CONNEXION MATÉRIELLE ---
            # Conversion vers l'enveloppe mécanique sûre confirmée pour le cou.
            servo_span = SERVO_MAX_ANGLE - SERVO_MIN_ANGLE
            real_angle = int(SERVO_MIN_ANGLE + body_state["last_servo_pos"] * servo_span)
            
            # On envoie l'ordre physique à l'Arduino
            if arduino_serial:
                try:
                    arduino_serial.write(f"M:{real_angle}\n".encode('utf-8'))
                except Exception as e:
                    pass

            # Métabolisme : bouger le moteur consomme de la batterie !
            ...
            # AFFICHAGE 
            if step % 5 == 0: # Rafraîchissement console légèrement ajusté
                thought = cortex.last_thought.split("->")[-1].strip() if hasattr(cortex, 'last_thought') else "En attente..."
                bat_pct = int(body_state['battery_level'] * 100)
                bat_bar = "█" * (bat_pct // 10)

                print(f"\r[{step}] Vie:{bat_bar} ({bat_pct}%) | Nociception:{body_state['collision_impact']:.2f} | Dist:{int(raw_dist)}cm | Surprise:{wm_error:.2f} | Angle:{real_angle:.2f} | Pensée: {thought}          ", end="", flush=True)

            step += 1
            
            # --- HORLOGE BIOLOGIQUE (20 Hz) ---
            # Calme le système pour se synchroniser avec le monde physique
            time.sleep(0.05) 
