...
from learning.resilience_001_agent import RecoveryAgent, state_digest
...
def source():
    return {**cumulative_source(), **{p: hashlib.sha256((ROOT / p).read_bytes()).hexdigest() for p in
        ['learning/resilience_001.py', 'learning/resilience_001_agent.py', 'cognitive/resilience.py',
         'learning/cumulative_001_choice.py', 'cognitive/memory.py', 'cognitive/kernel.py']}}


def create_manifest(path, bank='dev', count=6, variant='v2'):
    path = Path(path)
    ...
    manifest = {'namespace': ns, 'bank': bank, 'variant': variant, 'lives': lives, 'source': source(),
                'created_date': '2026-09-10', 'protocol': 'docs/research/resilience_001_preregistration.md',
                'protocol_sha256': hashlib.sha256((ROOT / 'docs/research/resilience_001_preregistration.md').read_bytes()).hexdigest(),
        ...
        for phase_index, (phase, organism) in enumerate(zip(PHASES, bodies)):
            ...
            for trial in range(12):
                ...
                if absolute == 13:
                    ...
                    kernel.close()
                    subprocess.run([sys.executable, '-m', 'learning.resilience_001', '--probe', str(request), str(response)],
